...
- Function `filter` from package [dplyr](https://dplyr.tidyverse.org/articles/dplyr.html#select-columns-with-select)). It using to filter the rows according to the condition that we want

```
books %>% filter(Rating > 4) %>% select(Rating, Book_title, Price)
```

- Function `arrange` from package [dplyr](https://dplyr.tidyverse.org/articles/dplyr.html#select-columns-with-select)). It using for sort rows

```
books %>% select(Rating, Book_title, Price) %>% arrange(Rating)
```

- Function `group_by` and `summarise` from package [dplyr](https://dplyr.tidyverse.org/articles/dplyr.html#select-columns-with-select)). 
  - `group_by` using for takes a data frame and one or more variables to group by
...
- Function `mutate` from package [dplyr](https://dplyr.tidyverse.org/articles/dplyr.html#select-columns-with-select)). It using for adds new variables that are functions of existing variablessort rows

```
books %>% mutate(Price_TH = Price*33)
```

- Function `ggplot` from package [ggplot](https://dplyr.tidyverse.org/articles/dplyr.html#select-columns-with-select)). It using creating various graphs
